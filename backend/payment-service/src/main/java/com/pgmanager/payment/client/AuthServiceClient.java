...
public class AuthServiceClient {
    ...
    public PgSettings getSettings(Long userId) {
        String url = authServiceUrl + "/settings/internal/" + userId;
        HttpHeaders headers = new HttpHeaders();
        headers.set("X-Gateway-Secret", internalSecret);
        HttpEntity<Void> entity = new HttpEntity<>(headers);
        try {
            ResponseEntity<PgSettings> response = restTemplate.exchange(url, HttpMethod.GET, entity, PgSettings.class);
